...
Market makers play a crucial role in modern financial markets by providing continuous liquidity, earning the bid-ask spread while managing two fundamental risks: inventory exposure and adverse selection [@GlostenMilgrom1985]. The challenge of adverse selection—being systematically picked off by informed traders—has intensified in the high-frequency trading era, where latency is measured in microseconds and information asymmetries can materialize and dissipate within milliseconds.
...
- **Inventory Risk**: ±100 shares maximum to limit overnight exposure (5x avg MM strategy size)
...
- OFI skew bounded by ±2$\sigma$ to avoid extreme quote deviation
...
- OFI Beta (β = 0.036): Estimated from an independent OFI replication study (Jan 2017, 40 symbol-days)
- Risk Aversion (γ = 0.1): Chosen from the canonical Avellaneda–Stoikov (2008) range
- OFI Signal Adjustment (α): Set using microstructure heuristics, not PnL optimization (hand-calibrated)
...
1. **Parameter Estimation Phase** (Separate Study):
   - OFI Beta (β = 0.036) estimated from an independent replication study (8 symbols, 40 symbol-days)
...
3. **Risk-Adjusted Edge**: The improvement shows in REDUCED LOSSES (-$3,352 → -$1,234) and LOWER VOLATILITY ($\sigma$: $6,440 → $2,382), not directional profits. OFI helps avoid the worst outcomes rather than catching the best moves.
...
Parametric fill simulation with exponential decay model λ($\delta$) = A·exp(-k·$\delta$) for backtest realism.
